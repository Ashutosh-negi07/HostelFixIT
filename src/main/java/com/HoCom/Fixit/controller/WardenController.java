...
import com.HoCom.Fixit.models.ComplaintStatusHistory;
...
import com.HoCom.Fixit.service.AuthService;
...
public class WardenController {
    ...
    @GetMapping("/students")
    public ResponseEntity<List<UserResponse>> getStudents(@AuthenticationPrincipal User currentUser) {
        if (currentUser.getHostel() == null) {
            throw new RuntimeException("Warden is not assigned to a hostel");
        }
        return ResponseEntity.ok(userService.getUsersByHostelAndRole(currentUser.getHostel().getId(), Role.STUDENT));
    }

    @GetMapping("/workers")
    public ResponseEntity<List<UserResponse>> getWorkers(@AuthenticationPrincipal User currentUser) {
        if (currentUser.getHostel() == null) {
            throw new RuntimeException("Warden is not assigned to a hostel");
        }
        return ResponseEntity.ok(userService.getUsersByHostelAndRole(currentUser.getHostel().getId(), Role.WORKER));
    }
    ...
    public ResponseEntity<List<ComplaintResponse>> getHostelComplaints(
        ...
        return ResponseEntity.ok(complaintService.getComplaintsByHostelFiltered(
                currentUser, statusEnum, priorityEnum, categoryId, sortBy, order));
    }
